...
export default function LearningEditor({ items, create, updateItems, activeId, onActiveChange }) {
  ...
  const removeCategory = (id, index, knowledgeCount) => {
    if (!window.confirm(`Delete this learning category${knowledgeCount ? ` and its ${knowledgeCount} knowledge point${knowledgeCount === 1 ? '' : 's'}` : ''}?`)) return;
