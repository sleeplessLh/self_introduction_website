...
export default function LearningEditor({ items, create, updateItems, activeId, onActiveChange }) {
  const updateCategory = (index, key, value) => updateItems(items.map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value } : item));
  const addCategory = () => { const item = create(); updateItems([...items, item]); onActiveChange(item.id); };
  const removeCategory = index => {
    if (!window.confirm('Delete this learning category?')) return;
  ...
  return <div className="collection-editor learning-editor">
    ...
    {items.map((item, index) => {
      ...
      const setKnowledge = value => updateCategory(index, 'knowledge', value);
      ...
      return <article className={`editor-item ${expanded ? 'is-active' : ''} ${item.hidden ? 'is-hidden' : ''}`} key={item.id}>
        <div className="editor-item-top"><button className="editor-item-select" onClick={() => onActiveChange(expanded ? null : item.id)}><strong>Category {index + 1}</strong><span>{item.title}</span></button><span><button onClick={() => updateItems(move(items, index, -1))}>↑</button><button onClick={() => updateItems(move(items, index, 1))}>↓</button><button onClick={() => updateCategory(index, 'hidden', !item.hidden)}>{item.hidden ? 'Show' : 'Hide'}</button><button className="danger" onClick={() => removeCategory(index)}>Delete</button></span></div>
        {expanded && <div className="editor-item-fields"><TextField label="Category title" value={item.title} onChange={value => updateCategory(index, 'title', value)} /><TextField label="Learning field" value={item.category || ''} onChange={value => updateCategory(index, 'category', value)} /><TextField label="Overall progress" value={item.status || ''} onChange={value => updateCategory(index, 'status', value)} /><TextField label="Period" value={item.period || ''} onChange={value => updateCategory(index, 'period', value)} /><TextField label="Category summary" value={item.summary || ''} multiline onChange={value => updateCategory(index, 'summary', value)} /><TagField label="Category tags" values={[...(item.topics || []), ...(item.technologies || [])]} onChange={value => updateItems(items.map((entry, entryIndex) => entryIndex === index ? { ...entry, topics: value, technologies: [] } : entry))} /><button className="editor-add secondary-add" onClick={() => setKnowledge([...knowledge, newKnowledge()])}>+ Add Knowledge Point</button>
