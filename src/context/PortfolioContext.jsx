...
const normalizeEducation = (items = []) => items.map(item => ({ id: item.id || stableId(), school: '', description: '', grade: '', startDate: '', endDate: '', ...item, experiences: (item.experiences || []).map(entry => typeof entry === 'string' ? { id: stableId(), text: entry } : { id: entry.id || stableId(), ...entry }) }));
const normalizeSkills = (groups = []) => groups.map(group => ({ id: group.id || stableId(), name: '', ...group, skills: (group.skills || []).map(skill => typeof skill === 'string' ? { id: stableId(), name: skill } : { id: skill.id || stableId(), ...skill }) }));
