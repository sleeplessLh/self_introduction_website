...
export default function Navbar() {
  ...
  const { content, update, hostMode, hostSession, setHostMode, requestHostAccess, requestEditor } = usePortfolio();
  ...
  return <header className={`navbar ${hidden ? 'is-hidden' : ''} ${open ? 'menu-open' : ''}`}>
    ...
    <details className="nav-more" ref={moreMenu}><summary className="more-button" role="button" aria-label="More options">⋯</summary><div className="more-menu" role="menu">{hostSession && <button role="menuitem" onClick={() => { closeMore(); requestEditor({ kind: 'content' }); }}>✎ Open Content Studio</button>}<button role="menuitem" onClick={hostSession ? exitHost : openHost}>{hostSession ? '↪ Exit Host Mode' : '▣ Host Access'}</button></div></details>
