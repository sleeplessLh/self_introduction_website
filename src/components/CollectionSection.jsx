...
export default function CollectionSection({ type }) {
  ...
  return <section className="projects-section compact-collection" id={sectionKey}><div className="projects-inner"><div className="section-label"><EditableText value={copy.label} onChange={value => setCopy('label', value)} /></div><Reveal className="projects-heading"><h2><EditableText value={copy.title} onChange={value => setCopy('title', value)} /><br /><em><EditableText value={copy.emphasis} onChange={value => setCopy('emphasis', value)} /></em></h2><p><EditableText value={copy.description} multiline onChange={value => setCopy('description', value)} /></p></Reveal>{collection.length ? <><div className="compact-list">{visible.map(item => <Reveal key={item.id}><CollectionCard item={item} type={type} detailLabel={copy.detailLabel} galleryLabel={copy.galleryLabel || 'View Gallery'} achievementLabel={copy.achievementLabel || 'Achievement'} onOpen={initialView => setSelected({ item, initialView })} /></Reveal>)}</div>{collection.length > 3 && <button className="collection-toggle" onClick={() => setExpanded(value => !value)}>{expanded ? copy.collapseLabel : copy.expandLabel} <b>{expanded ? '↑' : '↓'}</b></button>}</> : <div className="empty-collection"><EditableText value={copy.emptyLabel} onChange={value => setCopy('emptyLabel', value)} />{hostMode && <button className="editor-add collection-add" onClick={() => requestEditor({ kind: type, id: null })}>+ Add {singular}</button>}</div>}</div>{selected && <DetailModal item={selected.item} type={type} initialView={selected.initialView} achievementLabel={copy.achievementLabel || 'Achievement'} onClose={() => setSelected(null)} />}</section>;
}
