...
export default function LearningJourney() {
  ...
  return <section className="learning-section" id="learning"><div className="section">
    ...
    {entries.length ? <>
      <div className={`learning-list ${expanded ? 'is-expanded' : ''}`}>{entries.map(item => <Reveal key={item.id}><article className={`learning-item ${item.hidden ? 'is-hidden-host' : ''}`}><div><time className="learning-date"><EditableText value={item.date || 'Date / time range'} onChange={value => setItem(item.id, 'date', value)} /></time><h3><EditableText value={item.title} onChange={value => setItem(item.id, 'title', value)} /></h3><p><EditableText value={item.description || ''} multiline onChange={value => setItem(item.id, 'description', value)} /></p></div>{hostMode && <button onClick={() => requestEditor({ kind: 'learning', id: item.id })}>Edit item ↗</button>}</article></Reveal>)}</div>
